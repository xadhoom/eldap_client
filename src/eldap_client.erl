...
-export([open/1, open/2, simple_bind/3, simple_bind/4, controlling_process/2, start_tls/2,
         start_tls/3, start_tls/4, modify_password/3, modify_password/4, modify_password/5,
         getopts/2, baseObject/0, singleLevel/0, wholeSubtree/0, close/1, equalityMatch/2,
         greaterOrEqual/2, lessOrEqual/2, extensibleMatch/2, search/2, search/3, approxMatch/2,
         substrings/2, present/1, 'and'/1, 'or'/1, 'not'/1, mod_add/2, mod_delete/2, mod_replace/2,
         modify/3, modify/4, add/3, add/4, delete/2, delete/3, modify_dn/5, parse_dn/1,
         parse_ldap_url/1, paged_result_control/1, paged_result_control/2, paged_result_cookie/1]).
-export([neverDerefAliases/0, derefInSearching/0, derefFindingBaseObj/0, derefAlways/0]).
%% for upgrades
-export([loop/2]).

-import(lists, [concat/1]).

-include("ELDAPClientV3.hrl").
-include("eldap_client.hrl").
...
-record(eldap,
        {version = ?LDAP_VERSION,
         % Host running LDAP server
         host,
         % The LDAP server port
         port = ?LDAP_PORT,
         % Socket filedescriptor.
         fd,
         % Socket that was upgraded by start_tls
         prev_fd,
         % Name of the entry to bind as
         binddn = "",
         % Password for (above) entry
         passwd,
         % LDAP Request ID
         id = 0,
         ...
         % LDAP/LDAPS
         ldaps = false,
         % true if LDAPS or START_TLS executed
         using_tls = false,
         % ssl:ssloption()
         tls_opts = [],
         % inet6 support
...
add(Handle, Entry, Attributes, Controls)
    when is_pid(Handle), is_list(Entry), is_list(Attributes) ->
...
modify(Handle, Object, Mods, Controls)
    when is_pid(Handle), is_list(Object), is_list(Mods) ->
...
%%%            mod_replace("telephoneNumber", ["555 555 00"])
%%%
mod_add(Type, Values) when is_list(Type), is_list(Values) ->
    m(add, Type, Values).

mod_delete(Type, Values) when is_list(Type), is_list(Values) ->
    m(delete, Type, Values).

mod_replace(Type, Values) when is_list(Type), is_list(Values) ->
    m(replace, Type, Values).

m(Operation, Type, Values) ->
    #'ModifyRequest_changes_SEQOF'{operation = Operation,
                                   modification = #'PartialAttribute'{type = Type, vals = Values}}.
...
bool_p(Bool) when is_boolean(Bool) ->
    Bool.
...
%%%			 {attributes,["cn"]}])),
%%%
%%% Returned result:  {ok, #eldap_client_search_result{}}
%%%
%%% Example:
%%%
%%%  {ok,{eldap_client_search_result,
%%%        [{eldap_client_entry,
...
%%%
%%% --------------------------------------------------------------------
search(Handle, X) when is_pid(Handle), is_record(X, eldap_client_search); is_list(X) ->
    search(Handle, X, asn1_NOVALUE).

search(Handle, A, Controls) when is_pid(Handle), is_record(A, eldap_client_search) ->
    call_search(Handle, A, Controls);
search(Handle, L, Controls) when is_pid(Handle), is_list(L) ->
    case catch parse_search_args(L) of
        {error, Emsg} ->
            {error, Emsg};
        A when is_record(A, eldap_client_search) ->
            call_search(Handle, A, Controls)
    end.
...
parse_search_args(Args) ->
    parse_search_args(Args, #eldap_client_search{scope = wholeSubtree, deref = derefAlways}).

parse_search_args([{base, Base} | T], A) ->
    parse_search_args(T, A#eldap_client_search{base = Base});
parse_search_args([{filter, Filter} | T], A) ->
    parse_search_args(T, A#eldap_client_search{filter = Filter});
parse_search_args([{size_limit, SizeLimit} | T], A) when is_integer(SizeLimit) ->
    parse_search_args(T, A#eldap_client_search{size_limit = SizeLimit});
parse_search_args([{scope, Scope} | T], A) ->
    parse_search_args(T, A#eldap_client_search{scope = Scope});
parse_search_args([{deref, Deref} | T], A) ->
    parse_search_args(T, A#eldap_client_search{deref = Deref});
parse_search_args([{attributes, Attrs} | T], A) ->
    parse_search_args(T, A#eldap_client_search{attributes = Attrs});
parse_search_args([{types_only, TypesOnly} | T], A) ->
    parse_search_args(T, A#eldap_client_search{types_only = TypesOnly});
parse_search_args([{timeout, Timeout} | T], A) when is_integer(Timeout) ->
    parse_search_args(T, A#eldap_client_search{timeout = Timeout});
...
'not'(Filter) when is_tuple(Filter) ->
    {'not', Filter}.
...
extensibleMatch(MatchValue, OptArgs) ->
    MatchingRuleAssertion = mra(OptArgs, #'MatchingRuleAssertion'{matchValue = MatchValue}),
...
parse_args([{sslopts, Opts} | T], Cpid, Data) when is_list(Opts) ->
    parse_args(T,
               Cpid,
               Data#eldap{ldaps = true,
                          using_tls = true,
                          tls_opts = Opts ++ Data#eldap.tls_opts});
...
do_connect(Host, Data, Opts) when Data#eldap.ldaps == false ->
    gen_tcp:connect(Host, Data#eldap.port, Opts ++ Data#eldap.tcp_opts, Data#eldap.timeout);
    ...
    receive
        ...
        {From, {getopts, OptNames}} ->
            Result =
                try [case OptName of
                         port ->
                             {port, Data#eldap.port};
                         log ->
                             {log, Data#eldap.log};
                         timeout ->
                             {timeout, Data#eldap.timeout};
                         ssl ->
                             {ssl, Data#eldap.ldaps};
...
exec_extended_req_reply(Data, {ok, Msg})
...
%% For testing
do_simple_bind(Data, anon, anon, Controls) ->
...
exec_simple_bind_reply(Data, {ok, Msg})
    when Msg#'LDAPMessage'.messageID == Data#eldap.id ->
    case Msg#'LDAPMessage'.protocolOp of
        {bindResponse, Result} ->
...
do_search(Data, A, Controls) ->
    case catch do_search_0(Data, A, Controls) of
        ...
        {{ok, Val}, NewData} ->
            {{ok, Val}, NewData};
        {ok, Res, Rcontrols, Ref, NewData} ->
            {{ok, polish(Res, Rcontrols, Ref)}, NewData};
        {{error, Reason}, NewData} ->
            {{error, Reason}, NewData};
        Else ->
            {ldap_closed_p(Data, Else), Data}
    end.
...
polish(Res, Rcontrols, Ref) ->
    R = polish_result(Res),
    %%% Don't expose empty controls as ASN1 values
    C = case Rcontrols of
            asn1_NOVALUE ->
                [];
            Controls ->
    ...
    #eldap_client_search_result{entries = R,
                                referrals = Ref,
                                controls = C}.
...
polish_result([H | T]) when is_record(H, 'SearchResultEntry') ->
    ...
    [#eldap_client_entry{object_name = ObjectName, attributes = Attrs} | polish_result(T)];
polish_result([]) ->
    [].

do_search_0(Data, A, Controls) ->
    Req = #'SearchRequest'{baseObject = A#eldap_client_search.base,
                           scope = v_scope(A#eldap_client_search.scope),
                           derefAliases = v_deref(A#eldap_client_search.deref),
                           sizeLimit = v_size_limit(A#eldap_client_search.size_limit),
                           timeLimit = v_timeout(A#eldap_client_search.timeout),
                           typesOnly = v_bool(A#eldap_client_search.types_only),
                           filter = v_filter(A#eldap_client_search.filter),
                           attributes = v_attributes(A#eldap_client_search.attributes)},
    Id = bump_id(Data),
        ...
        {searchResDone, R} ->
            case R#'LDAPResult'.resultCode of
                success ->
                    ...
                    {ok, Acc, Msg#'LDAPMessage'.controls, Ref, Data};
                sizeLimitExceeded ->
...
do_add_0(Data, Entry, Attrs, Controls) ->
    Req = #'AddRequest'{entry = Entry, attributes = Attrs},
    S = Data#eldap.fd,
    Id = bump_id(Data),
...
do_modify_0(Data, Obj, Mod, Controls) ->
    v_modifications(Mod),
    Req = #'ModifyRequest'{object = Obj, changes = Mod},
    S = Data#eldap.fd,
    Id = bump_id(Data),
...
do_passwd_modify_0(Data, Dn, NewPasswd, OldPasswd, Controls) ->
    ...
    {ok, Bytes} = 'ELDAPClientV3':encode('PasswdModifyRequestValue', Req),
    ExtReq = #'ExtendedRequest'{requestName = ?PASSWD_MODIFY_OID, requestValue = Bytes},
    Id = bump_id(Data),
...
exec_passwd_modify_reply(Data, {ok, Msg})
    when Msg#'LDAPMessage'.messageID == Data#eldap.id ->
    case Msg#'LDAPMessage'.protocolOp of
        {extendedResp, Result} ->
            case Result#'ExtendedResponse'.resultCode of
                success ->
                    case Result#'ExtendedResponse'.responseValue of
                        asn1_NOVALUE ->
                            {ok, Data};
                        Value ->
                            case 'ELDAPClientV3':decode('PasswdModifyResponseValue', Value) of
...
do_unbind(Data) ->
    ...
    {no_reply,
     Data#eldap{binddn = (#eldap{})#eldap.binddn,
...
send_request(S, Data, Id, {T, P}) ->
    send_the_LDAPMessage(S, Data, #'LDAPMessage'{messageID = Id, protocolOp = {T, P}});
send_request(S, Data, Id, {T, P, asn1_NOVALUE}) ->
    send_the_LDAPMessage(S, Data, #'LDAPMessage'{messageID = Id, protocolOp = {T, P}});
send_request(S, Data, Id, {T, P, Controls0}) ->
    Controls =
        [#'Control'{controlType = F1,
                    criticality = F2,
                    controlValue = F3}
         || {control, F1, F2, F3} <- Controls0],
    send_the_LDAPMessage(S,
                         Data,
...
send_the_LDAPMessage(S, Data, LDAPMessage) ->
    {ok, Bytes} = 'ELDAPClientV3':encode('LDAPMessage', LDAPMessage),
...
recv_response(S, Data) ->
    case do_recv(S, Data, 0) of
        {ok, Packet} ->
            case 'ELDAPClientV3':decode('LDAPMessage', Packet) of
                {ok, Resp} ->
                    {ok, Resp};
                Error ->
                    throw(Error)
...
check_reply(Data, {ok, Msg}, Op) when Msg#'LDAPMessage'.messageID == Data#eldap.id ->
    case Msg#'LDAPMessage'.protocolOp of
        {Op, Result} ->
            case Result#'LDAPResult'.resultCode of
                success ->
                    {ok, Data};
                referral ->
                    {{ok, {referral, Result#'LDAPResult'.referral}}, Data};
                Error ->
                    {error, Error}
            end;
        Other ->
...
v_filter({'not', L}) ->
    {'not', L};
v_filter({equalityMatch, AV}) ->
    {equalityMatch, AV};
v_filter({greaterOrEqual, AV}) ->
    {greaterOrEqual, AV};
v_filter({lessOrEqual, AV}) ->
    {lessOrEqual, AV};
v_filter({approxMatch, AV}) ->
    {approxMatch, AV};
v_filter({present, A}) ->
    {present, A};
v_filter({substrings, S}) when is_record(S, 'SubstringFilter') ->
    {substrings, S};
...
v_scope(singleLevel) ->
    singleLevel;
v_scope(wholeSubtree) ->
    wholeSubtree;
v_scope(_Scope) ->
    throw({error, concat(["unknown scope: ", _Scope])}).

v_deref(DR = neverDerefAliases) ->
    DR;
v_deref(DR = derefInSearching) ->
    DR;
v_deref(DR = derefFindingBaseObj) ->
    DR;
v_deref(DR = derefAlways) ->
    DR.
...
v_size_limit(I) when is_integer(I), I >= 0 ->
    I;
v_size_limit(_I) ->
    throw({error, concat(["size_limit not positive integer: ", _I])}).

v_timeout(I) when is_integer(I), I >= 0 ->
    I;
v_timeout(_I) ->
    throw({error, concat(["timeout not positive integer: ", _I])}).

v_attributes(Attrs) ->
    F = fun (A) when is_list(A) ->
                A;
            (A) ->
...
% is a pair !
parse_string([$\\, X | T], Acc) when ?IS_SPECIAL(X) ->
...
parse_string([$\\, $" | T], Acc) ->
    parse_string(T, [$", $\\ | Acc]);
% is a pair!
parse_string([$\\, X, Y | T], Acc) when ?IS_HEXCHAR(X), ?IS_HEXCHAR(Y) ->
...
parse_keychars([H | T], Acc) when ?IS_ALPHA(H) ->
    parse_keychars(T, [H | Acc]);
parse_keychars([H | T], Acc) when ?IS_DIGIT(H) ->
    parse_keychars(T, [H | Acc]);
...
get_head([H | Tail], Tail, Rhead) ->
    lists:reverse([H | Rhead]);
get_head([H | Rest], Tail, Rhead) ->
...
-define(PAGE_CONTROL_OID, "1.2.840.113556.1.4.319").
...
paged_result_control(PageSize, Cookie) when is_integer(PageSize) ->
    RSCV = #'SearchControlValue'{size = PageSize, cookie = Cookie},
    {ok, ControlValue} = 'ELDAPClientV3':encode('SearchControlValue', RSCV),

    {control, ?PAGE_CONTROL_OID, true, ControlValue}.
...
paged_result_cookie(#eldap_client_search_result{controls = Controls}) ->
...
find_paged_result_cookie([C | Controls]) ->
    case C of
        #'Control'{controlType = ?PAGE_CONTROL_OID, controlValue = ControlValue} ->
            {ok, #'SearchControlValue'{cookie = Cookie}} =
                'ELDAPClientV3':decode('SearchControlValue', ControlValue),
            {ok, Cookie};
        _ ->
            find_paged_result_cookie(Controls)
    end.
