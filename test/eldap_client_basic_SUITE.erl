...
-module(eldap_client_basic_SUITE).

-export([add_already_exists/1, add_referral/1, add_when_bound/1, add_when_not_bound/1,
         app/1, appup/1, bind/1, client_side_add_timeout/1, client_side_bind_timeout/1,
         client_side_search_timeout/1, client_side_start_tls_timeout/1, close_after_tcp_error/1,
         close_ret_val/1, decode/1, delete/1, delete_referral/1, elementary_search/1, encode/1,
         modify/1, modify_dn_delete_old/1, modify_dn_keep_old/1, modify_referral/1, more_add/1,
         open_ret_val_error/1, open_ret_val_success/1, search_filter_and/1,
         search_filter_and_not/1, search_filter_equalityMatch/1,
         search_filter_equalityMatch_objectClass_exists/1, search_filter_final/1,
         search_filter_initial/1, search_filter_or/1, search_filter_or_sizelimit_ok/1,
         search_filter_or_sizelimit_exceeded/1, search_filter_substring_any/1,
         search_non_existant/1, search_referral/1, search_two_hits/1, search_paged_results/1,
         ssl_connection/1, start_tls_on_ssl_should_fail/1, start_tls_twice_should_fail/1,
         tcp_connection/1, tcp_connection_option/1]).
-export([all/0, end_per_group/2, end_per_suite/1, end_per_testcase/2, groups/0,
         init_per_group/2, init_per_suite/1, init_per_testcase/2, suite/0]).

%%-include_lib("common_test/include/ct.hrl").
-include_lib("common_test/include/ct.hrl").
-include_lib("eldap_client/include/eldap_client.hrl").
-include_lib("eldap_client/include/ELDAPClientV3.hrl").
...
groups() ->
    [{encode_decode, [], [encode, decode]},
     ...
     {api, [], [{group, api_not_bound}, {group, api_bound}]},
     {api_not_bound, [], [elementary_search, search_non_existant, add_when_not_bound, bind]},
     {api_bound,
      [],
      [add_when_bound,
       add_already_exists,
     ...
     {return_values, [], [open_ret_val_success, open_ret_val_error, close_ret_val]}].
...
init_per_suite(Config) ->
    ...
    [{ssl_available, SSL_available}, {ldap_server, LDAP_server}, {ldaps_server, LDAPS_server}
     | Config].
...
init_per_group(start_tls_api, Config0) ->
    case {proplists:get_value(ldap_server, Config0),
          proplists:get_value(ssl_available, Config0)}
    of
        {undefined, true} ->
            {skip, "LDAP server not availble"};
        {_, false} ->
        ...
        {Server = {Host, Port}, true} ->
            ...
            case supported_extension("1.3.6.1.4.1.1466.20037", Config) of
                true ->
                    initialize_db([{start_tls, true} | Config]);
                false ->
                    {skip, "start_tls not supported according to the server"}
            end
    end;
...
end_per_group(plain_api, Config) ->
    clear_db(Config);
end_per_group(ssl_api, Config) ->
    clear_db(Config);
end_per_group(start_tls_api, Config) ->
    clear_db(Config);
...
init_per_testcase(ssl_connection, Config) ->
    case proplists:get_value(ssl_available, Config) of
        true ->
            ...
            CertFile =
                filename:join(
                    proplists:get_value(data_dir, Config), "certs/server/cert.pem"),
            KeyFile =
                filename:join(
                    proplists:get_value(data_dir, Config), "certs/server/key.pem"),

            Parent = self(),
            Listener =
                spawn_link(fun() ->
                              case ssl:listen(SSL_Port,
            ...
            receive
                {ok, Listener} ->
                    ...
                    [{ssl_listener, Listener}, {ssl_listen_port, SSL_Port}, {ssl_connect_opts, []}
                     | Config];
        ...
        true ->
            case gen_tcp:listen(0, proplists:get_value(tcp_listen_opts, Config)) of
                {ok, LSock} ->
                    ...
                    [{listen_socket, LSock}, {listen_port, Port} | Config];
                Other ->
                    {fail, Other}
            end;
        false ->
            case proplists:get_value(name, proplists:get_value(tc_group_properties, Config)) of
                ...
                api_bound ->
                    {ok, H} = open(Config),
                    ok = eldap_client:simple_bind(H, "cn=Manager,dc=ericsson,dc=se", "hejsan"),
                    [{handle, H} | Config];
                _Name ->
                    Config
            end
...
end_per_testcase(_, Config) ->
              ...
              proplists:get_value(listen_socket, Config)),
    catch eldap_client:close(
              proplists:get_value(handle, Config)).
...
open_ret_val_success(Config) ->
    ...
    {ok, H} = eldap_client:open([Host], [{port, Port}]),
    catch eldap_client:close(H).

%%%----------------------------------------------------------------
open_ret_val_error(_Config) ->
    {error, _} = eldap_client:open(["nohost.example.com"], [{port, 65535}]).

%%%----------------------------------------------------------------
close_ret_val(Config) ->
    {Host, Port} = proplists:get_value(ldap_server, Config),
    {ok, H} = eldap_client:open([Host], [{port, Port}]),
    ok = eldap_client:close(H).
...
tcp_connection(Config) ->
    ...
    case eldap_client:open([Host], [{port, Port} | Opts]) of
        {ok, _H} ->
            ...
            case gen_tcp:accept(Sl, 1000) of
                ...
                Other ->
                    ct:fail("gen_tdp:accept failed: ~p", [Other])
            end;
        Other ->
            ct:fail("eldap_client:open failed: ~p", [Other])
    end.
...
close_after_tcp_error(Config) ->
    ...
    Opts = proplists:get_value(tcp_connect_opts, Config),
    T = 1000,
    case eldap_client:open([Host], [{timeout, T}, {port, Port} | Opts]) of
        {ok, H} ->
            Sl = proplists:get_value(listen_socket, Config),
            gen_tcp:close(Sl),
            {error, {gen_tcp_error, closed}} = eldap_client:simple_bind(H, anon, anon),
            ok = eldap_client:close(H),
            wait_for_close(H);
        Other ->
            ct:fail("eldap_client:open failed: ~p", [Other])
...
ssl_connection(Config) ->
    ...
    case eldap_client:open([Host],
                           [{port, Port}, {ssl, true}, {timeout, 5000}, {sslopts, SSLOpts} | Opts])
    of
        {ok, _H} ->
            ok;
        Other ->
...
client_side_add_timeout(Config) ->
    client_timeout(fun(H) ->
                      eldap_client:add(H,
                                       "cn=Foo Bar,dc=host,dc=ericsson,dc=se",
                                       [{"objectclass", ["person"]},
                                        {"cn", ["Foo Bar"]},
                                        {"sn", ["Bar"]},
...
client_side_bind_timeout(Config) ->
    client_timeout(fun(H) -> eldap_client:simple_bind(H, anon, anon) end, Config).

%%%----------------------------------------------------------------
client_side_search_timeout(Config) ->
    client_timeout(fun(H) ->
                      eldap_client:search(H,
                                          [{base, "dc=host,dc=ericsson,dc=se"},
                                           {filter, eldap_client:present("objectclass")},
                                           {scope, eldap_client:wholeSubtree()}])
                   end,
                   Config).

%%%----------------------------------------------------------------
client_side_start_tls_timeout(Config) ->
    client_timeout(fun(H) -> eldap_client:start_tls(H, []) end, Config).
...
tcp_connection_option(Config) ->
    ...
    case catch eldap_client:open([Host],
                                 [{port, Port}, {tcpopts, [{linger, TestLinger}]} | Opts])
    of
        {ok, H} ->
            case gen_tcp:accept(Sl, 1000) of
                {ok, _} ->
                    case eldap_client:getopts(H, [{tcpopts, [linger]}]) of
                        {ok, [{tcpopts, [{linger, ActualLinger}]}]} ->
                            case ActualLinger of
                                TestLinger ->
                                    ok;
                                DefaultLinger ->
                                    ct:fail("eldap_client:getopts: 'linger' didn't change,"
                                            " got ~p (=default) expected ~p",
                                            [ActualLinger, TestLinger]);
                                _ ->
                                    ct:fail("eldap_client:getopts: bad 'linger', got ~p expected ~p",
                                            [ActualLinger, TestLinger])
                            end;
                        Other ->
                            ct:fail("eldap_client:getopts: bad result ~p", [Other])
                    end;
                {error, timeout} ->
                    ct:fail("server side accept timeout", [])
            end;
...
elementary_search(Config) ->
    {ok, #eldap_client_search_result{entries = [_]}} =
        eldap_client:search(
            proplists:get_value(handle, Config),
            #eldap_client_search{base = proplists:get_value(eldap_path, Config),
                                 filter = eldap_client:present("objectclass"),
                                 scope = eldap_client:wholeSubtree()}).
...
search_non_existant(Config) ->
    {error, noSuchObject} =
        eldap_client:search(
            proplists:get_value(handle, Config),
            #eldap_client_search{base = "cn=Bar," ++ proplists:get_value(eldap_path, Config),
                                 filter = eldap_client:present("objectclass"),
                                 scope = eldap_client:wholeSubtree()}).
...
add_when_not_bound(Config) ->
    {error, _} =
        eldap_client:add(
            ...
            [{"objectclass", ["person"]}, {"cn", ["Jonas Jonsson"]}, {"sn", ["Jonsson"]}]).

%%%----------------------------------------------------------------
bind(Config) ->
    ok =
        eldap_client:simple_bind(
            proplists:get_value(handle, Config), "cn=Manager,dc=ericsson,dc=se", "hejsan").
...
add_referral(Config) ->
    ...
    {ok, {referral, ["ldap://nowhere.example.com" ++ _]}} =
        eldap_client:add(H,
                         "cn=Foo Bar,dc=notHere," ++ BasePath,
                         [{"objectclass", ["person"]},
                          {"cn", ["Foo Bar"]},
                          {"sn", ["Bar"]},
                          {"telephoneNumber", ["555-1232", "555-5432"]}]).
...
search_filter_equalityMatch(Config) ->
    ...
    {ok,
     #eldap_client_search_result{entries = [#eldap_client_entry{object_name = ExpectedDN}]}} =
        eldap_client:search(
            proplists:get_value(handle, Config),
            #eldap_client_search{base = BasePath,
                                 filter = eldap_client:equalityMatch("sn", "Jonsson"),
                                 scope = eldap_client:singleLevel()}).
...
search_filter_equalityMatch_objectClass_exists(Config) ->
    ...
    {ok,
        ...
        eldap_client:search(
            ...
            #eldap_client_search{base = BasePath,
                                 filter =
                                     eldap_client:'and'([eldap_client:equalityMatch("sn",
                                                                                    "Jonsson"),
                                                         eldap_client:present("objectclass")]),
                                 scope = eldap_client:singleLevel()}).
...
search_filter_substring_any(Config) ->
    ...
    {ok,
        ...
        eldap_client:search(
            proplists:get_value(handle, Config),
            #eldap_client_search{base = BasePath,
                                 filter = eldap_client:substrings("sn", [{any, "ss"}]),
                                 scope = eldap_client:singleLevel()}).
...
search_filter_final(Config) ->
    ...
    {ok,
        ...
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:substrings("sn", [{final, "r"}]),
                                                 scope = eldap_client:singleLevel()}).
...
search_filter_and(Config) ->
    ...
    {ok,
        ...
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:'and'([eldap_client:substrings("sn",
                                                                                                 [{any,
                                                                                                   "a"}]),
                                                                         eldap_client:equalityMatch("cn",
                                                                                                    "Foo Bar")]),
                                                 scope = eldap_client:singleLevel()}).
...
search_filter_or(Config) ->
    H = proplists:get_value(handle, Config),
    BasePath = proplists:get_value(eldap_path, Config),
    ExpectedDNs = lists:sort(["cn=Foo Bar," ++ BasePath, "ou=Team," ++ BasePath]),
    {ok, #eldap_client_search_result{entries = Es}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:'or'([eldap_client:substrings("sn",
                                                                                                [{any,
                                                                                                  "a"}]),
                                                                        eldap_client:equalityMatch("ou",
                                                                                                   "Team")]),
                                                 scope = eldap_client:singleLevel()}),
    ExpectedDNs = lists:sort([DN || #eldap_client_entry{object_name = DN} <- Es]).
...
search_filter_or_sizelimit_exceeded(Config) ->
    ...
    ExpectedDNs = ["cn=Foo Bar," ++ BasePath, "ou=Team," ++ BasePath],
    %% Expect exact one of the two answers, but we don't know which:
    {ok, #eldap_client_search_result{entries = [E]}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:'or'([eldap_client:substrings("sn",
                                                 ...
                                                 size_limit = 1,
                                                 scope = eldap_client:singleLevel()}),
    #eldap_client_entry{object_name = DN} = E,
...
search_filter_and_not(Config) ->
    H = proplists:get_value(handle, Config),
    BasePath = proplists:get_value(eldap_path, Config),
    {ok, #eldap_client_search_result{entries = []}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:'and'([eldap_client:substrings("sn",
...
search_two_hits(Config) ->
    ...
    {ok, #eldap_client_search_result{entries = Es}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter = eldap_client:present("description"),
                                                 scope = eldap_client:singleLevel()}),

    %% And check that they are the expected ones:
    ExpectedDNs = lists:sort([DN1, DN2]),
    ExpectedDNs = lists:sort([D || #eldap_client_entry{object_name = D} <- Es]),

    %% Restore the database:
    [ok = eldap_client:delete(H, DN) || DN <- ExpectedDNs].
...
search_paged_results(Config) ->
    ...
    DNs = [{"cn=Jeremy " ++ N ++ "," ++ BasePath,
            [{"objectclass", ["person"]},
             {"cn", ["Jeremy " ++ N]},
             {"sn", [N]},
             {"description", [Desc]}]}
           || N <- Names],
    [ok = eldap_client:add(H, Entry, Attrs) || {Entry, Attrs} <- DNs],

    PageSize = 10,

    Control1 = eldap_client:paged_result_control(PageSize),

    {ok, SearchResult1} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:equalityMatch("description",
                                                                                Desc),
                                                 scope = eldap_client:singleLevel()},
                            [Control1]),

    #eldap_client_search_result{entries = Es1} = Res = SearchResult1,

    PageSize = length(Es1),

    {ok, Cookie1} = eldap_client:paged_result_cookie(SearchResult1),

    Control2 = eldap_client:paged_result_control(PageSize, Cookie1),
    ...
    #eldap_client_search_result{entries = Es2} = SearchResult2,

    PageSize = length(Es2),

    %% all results have been returned so cookie should be empty
    {ok, []} = eldap_client:paged_result_cookie(SearchResult2),

    ExpectedDNs = lists:sort([DN || {DN, _} <- DNs]),
    ResultDNs = lists:sort([DN || #eldap_client_entry{object_name = DN} <- Es1 ++ Es2]),

    ExpectedDNs = ResultDNs,

    %% Restore the database:
    [ok = eldap_client:delete(H, DN) || {DN, _} <- DNs].
...
modify(Config) ->
    ...
    %% Do a change
    Mod = [eldap_client:mod_replace("telephoneNumber", ["555-12345"]),
           eldap_client:mod_add("description", ["Nice guy"])],
    ok = eldap_client:modify(H, DN, Mod),

    %% Check that the object was changed
    {ok, #eldap_client_search_result{entries = [#eldap_client_entry{object_name = DN}]}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:equalityMatch("telephoneNumber",
                                                                                "555-12345"),
                                                 scope = eldap_client:singleLevel()}),

    %% Do another type of change
    ok = eldap_client:modify(H, DN, [eldap_client:mod_delete("telephoneNumber", [])]),
...
modify_referral(Config) ->
    ...
    Mod = [eldap_client:mod_replace("telephoneNumber", ["555-12345"]),
           eldap_client:mod_add("description", ["Nice guy"])],
    {ok, {referral, ["ldap://nowhere.example.com" ++ _]}} = eldap_client:modify(H, DN, Mod).
...
delete(Config) ->
    ...
    ok = eldap_client:delete(H, DN),
    %% check that it really was deleted:
    {error, noSuchObject} = eldap_client:delete(H, DN),
...
delete_referral(Config) ->
    ...
    {ok, {referral, ["ldap://nowhere.example.com" ++ _]}} = eldap_client:delete(H, DN).
...
modify_dn_delete_old(Config) ->
    ...
    CN_orig =
        lists:sort(
            proplists:get_value("cn", OriginalAttrs)),
    {ok, #eldap_client_search_result{entries = [#eldap_client_entry{object_name = DN}]}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:substrings("sn", [{any, "a"}]),
                                                 scope = eldap_client:singleLevel()}),

    %% Modify and delete the old one:
    ok = eldap_client:modify_dn(H, DN, NewRDN, true, ""),

    %% Check that DN was modified and the old one was deleted:
    {ok, NewAttrs} = attributes(H, NewDN),
    CN_new =
        lists:sort(
            proplists:get_value("cn", NewAttrs)),
    {ok, #eldap_client_search_result{entries = [#eldap_client_entry{object_name = NewDN}]}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
                                                     eldap_client:substrings("sn", [{any, "a"}]),
    ...
    %% Change back:
    ok = eldap_client:modify_dn(H, NewDN, OriginalRDN, true, ""),

    %% Check that DN was modified and the new one was deleted:
    {ok, SameAsOriginalAttrs} = attributes(H, DN),
    CN_orig =
        lists:sort(
            proplists:get_value("cn", SameAsOriginalAttrs)),
    {ok, #eldap_client_search_result{entries = [#eldap_client_entry{object_name = DN}]}} =
        eldap_client:search(H,
                            #eldap_client_search{base = BasePath,
                                                 filter =
...
modify_dn_keep_old(Config) ->
    ...
    ok = eldap_client:modify_dn(H, DN, NewRDN, false, ""),
    ...
    %% Restore db:
    ok = eldap_client:delete(H, NewDN),
    restore_original_object(H, DN, OriginalAttrs).
...
start_tls_twice_should_fail(Config) ->
    ...
    {error, tls_already_started} = eldap_client:start_tls(H, []),
    eldap_client:close(H).
...
encode(_Config) ->
    {ok, Bin} =
        'ELDAPClientV3':encode('AddRequest', #'AddRequest'{entry = "hejHopp", attributes = []}),
    Expected = <<104, 11, 4, 7, 104, 101, 106, 72, 111, 112, 112, 48, 0>>,
    case Bin of
        Expected ->
            ok;
        _ ->
            ct:log("Encoded erroneously to:~n~p~nExpected:~n~p", [Bin, Expected]),
            {fail, "Bad encode"}
    end.
...
decode(_Config) ->
    {ok, Res} =
        'ELDAPClientV3':decode('AddRequest',
                               <<104, 11, 4, 7, 104, 101, 106, 72, 111, 112, 112, 48, 0>>),
    ct:log("Res = ~p", [Res]),
...
attributes(H, DN) ->
    case eldap_client:search(H,
                             #eldap_client_search{base = DN,
                                                  filter = eldap_client:present("objectclass"),
                                                  scope = eldap_client:wholeSubtree()})
    of
        {ok,
         #eldap_client_search_result{entries =
                                         [#eldap_client_entry{object_name = DN,
...
restore_original_object(H, DN, Attrs) ->
    eldap_client:delete(H, DN),
    ok = eldap_client:add(H, DN, Attrs).
...
find_first_server(UseSSL, [{Host, Port} | Ss]) ->
    case eldap_client:open([Host], [{port, Port}, {ssl, UseSSL}, {timeout, 10000}]) of
        {ok, H} when UseSSL == false, Ss =/= [] ->
            case eldap_client:start_tls(H, [], 10000) of
                ok ->
                    ct:log("find_first_server ~p UseSSL=~p -> ok", [{Host, Port}, UseSSL]),
                    eldap_client:close(H),
                    {Host, Port};
                Res ->
                    ct:log("find_first_server ~p UseSSL=~p failed with~n~p~nSave as spare host.",
                           [{Host, Port}, UseSSL, Res]),
                    eldap_client:close(H),
...
find_first_server(false, [{spare_host, Host, Port} | _]) ->
    ct:log("find_first_server can't find start_tls host, use the spare non-start_tls host for plain ldap: ~p",
           [{Host, Port}]),
    {Host, Port};
find_first_server(_, []) ->
...
delete_old_contents(H, Path) ->
    case eldap_client:search(H,
                             [{base, Path},
                              ...
                              {scope, eldap_client:wholeSubtree()}])
    of
        {ok, _R = #eldap_client_search_result{entries = Entries}} ->
            case eldap_client:delete(H, "dc=notHere," ++ Path, [?manageDsaIT]) of
                ok ->
                    ok;
                {error, noSuchObject} ->
                    ok;
                Other ->
                    ct:fail("eldap_client:delete notHere ret ~p", [Other])
            end,
            [ok = eldap_client:delete(H, DN) || #eldap_client_entry{object_name = DN} <- Entries];
        _Res ->
            ignore
    end.
...
add_new_contents(H, Path, MyHost) ->
    ...
    ?ok(eldap_client:add(H,
                         "dc=notHere," ++ Path,
                         [{"objectclass", ["referral", "dcObject"]},
                          {"ref", ["ldap://nowhere.example.com/notHere," ++ Path]},
                          {"dc", ["notHere"]}])).

ok(_, _, {error, entryAlreadyExists}) ->
    ok;
ok(_, _, ok) ->
    ok;
ok(MODULE, LINE, X) ->
    ct:pal("~p:~p add_new_contents: ret from eldap_client:add = ~p", [MODULE, LINE, X]),
    X.
...
start_tls(H, Config) ->
    KeyFile = filename:join([proplists:get_value(data_dir, Config), "certs/client/key.pem"]),
    case eldap_client:start_tls(H, [{keyfile, KeyFile}]) of
        ok ->
...
open(Config) ->
    ...
    {ok, H} = eldap_client:open([Host], [{port, Port}, {ssl, SSLflag}]),
    cond_start_tls(H, Config),
    {ok, H}.
...
supported_extension(OID, Config) ->
    {ok, H} = open_bind(Config),
    case eldap_client:search(H,
                             [{scope, eldap_client:baseObject()},
                              {filter, eldap_client:present("objectclass")},
                              {deref, eldap_client:neverDerefAliases()},
                              {attributes, ["+"]}])
    of
        {ok, R = #eldap_client_search_result{}} ->
            eldap_client:close(H),
            lists:member(OID,
                         [SE
                          || EE <- R#eldap_client_search_result.entries,
                             {"supportedExtension", SEs} <- EE#eldap_client_entry.attributes,
                             SE <- SEs]);
        _ ->
            eldap_client:close(H),
            false
...
ms_passed(T0) ->
    %% OTP 18
    erlang:convert_time_unit(erlang:monotonic_time() - T0, native, micro_seconds) / 1000.
    ...
    try ssl:start() of
        R when R == ok; R == {error, {already_started, ssl}} ->
            try make_certs:all("/dev/null",
                               filename:join(
                                   proplists:get_value(data_dir, Config), "certs"))
            of
                {ok, _} ->
                    true;
                Other ->
