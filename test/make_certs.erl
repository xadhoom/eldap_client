...
all(DataDir, PrivDir, C = #config{}) ->
    ok =
        filelib:ensure_dir(
            filename:join(PrivDir, "erlangCA")),
...
rootCA(Root, Name, C) ->
    ...
    file:copy(
        filename:join([Root, Name, "cert.pem"]), filename:join([Root, Name, "cacerts.pem"])),
    gencrl(Root, Name, C).
...
intermediateCA(Root, CA, ParentCA, C) ->
    ...
    file:copy(
        filename:join([Root, ParentCA, "cacerts.pem"]), CACertsFile),
...
enduser(Root, CA, User, C) ->
    ...
    file:copy(
        filename:join([Root, CA, "cacerts.pem"]), CACertsFile),
    ok.

revoke(Root, CA, User, C) ->
    UsrCert = filename:join([Root, User, "cert.pem"]),
    CACnfFile = filename:join([Root, CA, "ca.cnf"]),
    Cmd = [C#config.openssl_cmd,
           " ca"
           " -revoke ",
           UsrCert,
...
verify(Root, CA, User, C) ->
    ...
    Cmd = [C#config.openssl_cmd,
           ...
           CAFile,
           " -CRLfile ",
           CACRLFile, %% this is undocumented, but seems to work
           " -crl_check ",
           CertFile],
    Env = [{"ROOTDIR", filename:absname(Root)}],
    try
...
create_ca_dir(Root, CAName, Cnf) ->
    ...
    create_files(CARoot,
                 [{"serial", "01\n"}, {"crlnumber", "01"}, {"index.txt", ""}, {"ca.cnf", Cnf}]).
...
create_req(Root, CnfFile, KeyFile, ReqFile, C) ->
    Cmd = [C#config.openssl_cmd,
           ...
           ReqFile],
    Env = [{"ROOTDIR", filename:absname(Root)}],
    cmd(Cmd, Env).    %fix_key_file(KeyFile).
...
cmd(Cmd, Env) ->
    ...
    Port = open_port({spawn, FCmd}, [stream, eof, exit_status, stderr_to_stdout, {env, Env}]),
    eval_cmd(Port, FCmd).
...
req_cnf(C) ->
    ["# Purpose: Configuration for requests (end users and CAs)."
     ...
     "default_bits	= ",
     integer_to_list(C#config.default_bits),
     "\n"
     ...
     C#config.organizationalUnitName,
     "\n"
     "organizationName	        = ",
     C#config.organizationName,
     "\n"
     "localityName		= ",
     C#config.localityName,
     "\n"
     "countryName		= ",
     C#config.countryName,
     "\n"
     "emailAddress		= ",
     C#config.emailAddress,
     "\n"
     "\n"
     "[ca_ext]\n"
...
ca_cnf(C) ->
    ["# Purpose: Configuration for CAs.\n"
     ...
     "[idpsec]\n"
     "fullname=URI:http://localhost:8000/",
     C#config.commonName,
     "/crl.pem\n"
     "[user_cert]\n"
     ...
     C#config.commonName,
     "/crl.pem\n"
     "URI.2=http://localhost:",
     integer_to_list(C#config.crl_port),
     "/",
     C#config.commonName,
     "/crl.pem\n"
     "\n"
