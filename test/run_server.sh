...
if [ ! -d "$__dir__/eldap_client_basic_SUITE_data/certs" ]; then
    echo "Creating certs in $__dir__/eldap_client_basic_SUITE_data/certs"
    (
        cd $__dir__ \
        && erlc make_certs.erl \
        && erl -noinput -eval 'make_certs:all("/dev/null", "eldap_client_basic_SUITE_data/certs").' -s init stop
    )
fi

docker run \
    --rm \
    -v "${__dir__}/eldap_client_basic_SUITE_data/certs:/opt/otp/openldap/certs":z \
